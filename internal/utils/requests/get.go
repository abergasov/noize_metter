...
func GetCurl[T any](ctx context.Context, targetURL string, headers map[string]string, opts ...Option) (res T, statusCode int, err error) {
	...
	req, err := http.NewRequestWithContext(c, http.MethodGet, targetURL, http.NoBody)
	if err != nil {
	...
	return WrappedRun[T](req, config)
}

func GetCurlText(ctx context.Context, targetURL string, headers map[string]string, opts ...Option) (res string, statusCode int, err error) {
	config := NewDefaultConfig()
